...
import { DEFAULT_TRANSLATION, QURAN_API_BASE_URL } from "./quran.constant";

import { Ayah, Surah } from "./quran.model";

const generateAudioUrl = (surahNumber: number, ayahNumber: number) => {
...
const generateFullSurahAudio = (surahNumber: number) => {
...
const getSurahStatsFromDB = async () => {
  try {
    ...
    const longestSurah = await Surah.findOne({
      isDeleted: false,
    })
      .sort({
        totalAyahs: -1,
    ...
    const shortestSurah = await Surah.findOne({
      isDeleted: false,
    })
      .sort({
        totalAyahs: 1,
